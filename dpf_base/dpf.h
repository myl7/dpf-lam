...
uint128_t salsa20_12(uint128_t seed, uint128_t pos) {
  ...
  uint128_t result = 0;
  result = ((uint128_t)out[1] << 96) + ((uint128_t)out[2] << 64) + ((uint128_t)out[3] << 32) + ((uint128_t)out[4] << 0);
  return result;
}
...
uint128_t chacha20_12(uint128_t seed, uint128_t pos) {
  ...
  // Use upper half as result of PRF
  uint128_t result = 0;
  result = ((uint128_t)out[4] << 96) + ((uint128_t)out[5] << 64) + ((uint128_t)out[6] << 32) + ((uint128_t)out[7] << 0);
  return result;
}
...
uint128_t aes128(uint128_t seed, uint128_t pos) {
  ...
  u64 w[22] = {0};
  KeyExpansion((const unsigned char *)&seed, w, nr, 4);
...
uint128_t (*PRF_SELECT(int prf_method))(uint128_t, uint128_t) {
  ...
  if (prf_method == DUMMY)
    PRF = K;
  else if (prf_method == SALSA)
    PRF = salsa20_12;
  else if (prf_method == CHACHA)
    PRF = chacha20_12;
...
SeedsCodewords *GenerateSeedsAndCodewords(
  int alpha, uint128_t beta, int N, int n_keys, int n_codewords, std::mt19937 &g_gen, int prf_method) {
  assert(alpha < N);
...
size_t SizeOf(const SeedsCodewords *s) {
  ...
  size_t s1 = sizeof(s);
  size_t s2 = (s->codewords_1.size() + s->codewords_2.size() + s->k1.size()) * sizeof(s->k1[0]);
  return s1 + s2 + rest;
}
