...
class DPF(object):
    ...
    def gen(self, k, n, seed: bytes | None = None):
        ...
        if k >= n:
            raise Exception(
                "k (%d), the selected element, must be less than n (%d), the number of entries in the table" % (k, n)
            )
    ...
    def eval_init(self, table):
        ...
        if self.table_effective_entry_size > self.ENTRY_SIZE:
            raise Exception(
                "Table entry dimension (%d) must be < %d" % (self.table_effective_entry_size, self.ENTRY_SIZE)
            )
    ...
    def __repr__(self):
        if self.buffers is None:
            ...
            return "DPF(entries=%d, entry_size=%d, prf_method=%s)" % (
                ...
                self.table_effective_entry_size,
                self.prf_method_string,
            )
