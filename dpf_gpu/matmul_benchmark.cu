...
void alloc_test_matrix(uint128_t_gpu **A_gpu, uint128_t_gpu **A_cpu, int M, int N) {
...
void check_correct(uint128_t_gpu *A, uint128_t_gpu *B, uint128_t_gpu *C, int M, int K, int N) {
  ...
  for (int i = 0; i < M; i++) {
    for (int j = 0; j < N; j++) {
      ...
      assert(got.x == expected.x && got.y == expected.y && got.z == expected.z && got.w == expected.w);
    }
  }
...
int main(void) {
  ...
  printf(
    "{'entries (K)': %d, 'entry_size_ints (M)': %d, 'batch_size (N)': %d,"
    "'latency_ms' : %f, 'throughput_queries_per_ms' : %f'}\n",
    KK, MM, NN, ms_latency, throughput_per_query);
