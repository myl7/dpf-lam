...
void dpf_hybrid_deinitialize() {
  cudaFree(DPF_HYBRID_STACK_1);
  cudaFree(DPF_HYBRID_STACK_2);
}
...
__global__ void dpf_hybrid_kernel(SeedsCodewordsFlatGPU *cw, uint128_t_gpu *out, uint128_t_gpu *DPF_HYBRID_STACK_1,
  uint128_t_gpu *DPF_HYBRID_STACK_2, uint128_t_gpu *TABLE, int batch_size, int num_entries) {
  /////////////////////////////////////////////////////////////////////////
  ...
  for (int i = cw_shared[0].depth - 1; i >= cw_shared[0].depth - LOGZ; i--) {
    ...
    for (int j = start; j < end; j += parallel_work_per_threadblock_per_dpf) {
      ...
      if (expansion_idx < end) {
        ...
        uint128_t_gpu value = PRF<prf_method>(key, idx_into_codewords);
        ...
        key_write[expansion_idx + (block_idx + batch_idx) * max_stack_size_per_dpf] =
          add_uint128(value, cw[idx_into_codewords]);
      }
    }
  }
    ...
    if (cur_depth == cw_shared[0].depth) {
      ...
      for (int i = 0; i < Z; i += parallel_work_per_threadblock_per_dpf) {
        ...
        if (expansion_idx < Z) {
          ...
          out[write_out + expansion_idx + batch_idx * num_entries +
              block_idx * DPF_HYBRID_DPFS_PER_BLOCK * num_entries] =
          ...
          uint128_t_gpu dpf_output =
            key_read[cw_read_write_indx_strt + expansion_idx + (block_idx + batch_idx) * max_stack_size_per_dpf];
          for (int z = 0; z < MM; z++) {
            per_thread_accumulate[z] = add_uint128(
              mul_uint128(dpf_output, TABLE[write_out + expansion_idx + z * num_entries]), per_thread_accumulate[z]);
          }
#endif
        }
      }
      ...
      for (int j = start; j < end; j += parallel_work_per_threadblock_per_dpf) {
        ...
        if (expansion_idx < end) {
          ...
          uint128_t_gpu key =
            key_read[cw_read_write_indx_strt + (expansion_idx / 2) + (block_idx + batch_idx) * max_stack_size_per_dpf];
          uint128_t_gpu value = PRF<prf_method>(key, idx_into_codewords);
          uint128_t_gpu *cw = (key.x & 1) == 0 ? cw_shared[batch_idx].cw_1 : cw_shared[batch_idx].cw_2;
  ...
  for (int element = 0; element < MM; element++) {
    ...
    for (int neighbor = 1; neighbor < DPF_HYBRID_THREADS_PER_BLOCK; neighbor *= 2) {
      if (thread_idx % (neighbor * 2) == 0 && thread_idx + neighbor < DPF_HYBRID_THREADS_PER_BLOCK) {
        shared_accumulates[thread_idx] =
          add_uint128(shared_accumulates[thread_idx], shared_accumulates[thread_idx + neighbor]);
      }
      __syncthreads();
    }
...
template <int prf_method>
void dpf_hybrid(SeedsCodewordsFlatGPU *cw, uint128_t_gpu *out, uint128_t_gpu *TABLE, int batch_size, int num_entries,
  cudaStream_t s) {
  dim3 n_blocks(batch_size / DPF_HYBRID_DPFS_PER_BLOCK);
  dim3 n_threads(DPF_HYBRID_THREADS_PER_BLOCK);
  dpf_hybrid_kernel<prf_method>
    <<<n_blocks, n_threads, 0, s>>>(cw, out, DPF_HYBRID_STACK_1, DPF_HYBRID_STACK_2, TABLE, batch_size, num_entries);
}
