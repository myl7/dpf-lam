...
__global__ void dpf_coop_kernel(SeedsCodewordsFlatGPU *cw, uint128_t_gpu *TABLE, uint128_t_gpu *TABLE_REDUCTION,
  uint128_t_gpu *out, uint128_t_gpu *DPF_COOP_KEYS_1, uint128_t_gpu *DPF_COOP_KEYS_2, int batch_size, int num_entries,
    ...
    if (expansion_idx < num_entries) {
      for (int z = 0; z < MM; z++) {
        per_thread_accumulate[z] =
          add_uint128(mul_uint128(key_write[expansion_idx], TABLE[expansion_idx]), per_thread_accumulate[z]);
      }
    }
  }
...
void dpf_coop(SeedsCodewordsFlatGPU *cw, uint128_t_gpu *out, uint128_t_gpu *TABLE, int batch_size, int num_entries,
  cudaStream_t s) {
  ...
  cudaLaunchCooperativeKernel((void *)dpf_coop_kernel, n_blocks, n_threads, kernel_args);
}
