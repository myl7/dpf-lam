...
__device__ uint128_t_gpu PRF_DUMMY(uint128_t_gpu seed, uint32_t i) {
  ...
  return add_uint128(mul_uint128(seed, add_uint128(val_4242, val_i)), add_uint128(val_4242, val_i));
}
...
#define QR_CHA(a, b, c, d) \
  (a += b, d ^= a, d = ROTL_CHA(d, 16), c += d, b ^= c, b = ROTL_CHA(b, 12), a += b, d ^= a, d = ROTL_CHA(d, 8), \
...
__device__ uint128_t_gpu aes128_gpu(uint128_t_gpu seed, uint32_t pos) {
  ...
  // Key expansion
  AES_KEY k;
  AES_set_encrypt_key((const unsigned char *)&seed, 128, &k);
