...
uint128_t uint128_from_gpu(uint128_t_gpu val) {
  uint128_t res = 0;
  return val.x + ((uint128_t)val.y << 32) + ((uint128_t)val.z << 64) + ((uint128_t)val.w << 96);
}

__device__ uint128_t_gpu uint128_from(uint64_t hi, uint64_t lo) {
  uint128_t_gpu res;
  res.w = (hi >> 32);
...
__device__ uint128_t_gpu add_uint128(uint128_t_gpu addend, uint128_t_gpu augend) {
  ...
  asm(
    ...
    : "r"(addend.x), "r"(addend.y), "r"(addend.z), "r"(addend.w), "r"(augend.x), "r"(augend.y), "r"(augend.z),
    "r"(augend.w));
  return res;
}
...
void check_correct(
  SeedsCodewordsFlat *cw, uint128_t_gpu *target, int batch_size, int num_entries, int permutated_ordering) {
  int zz = 0;
  for (int i = 0; i < batch_size; i++) {
    for (int j = 0; j < num_entries; j++) {
      ...
      assert(got.x == truth_128_t_gpu.x && got.y == truth_128_t_gpu.y && got.z == truth_128_t_gpu.z &&
...
void check_correct_fused(SeedsCodewordsFlat *cw, uint128_t_gpu *target, uint128_t_gpu *table, int entry_size,
  ...
  for (int i = 0; i < batch_size; i++) {
    for (int k = 0; k < entry_size; k++) {
      ...
      assert(got.x == cmp.x && got.y == cmp.y && got.z == cmp.z && got.w == cmp.w);
    }
  }
