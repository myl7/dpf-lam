...
__global__ void GEMM128_kernel(
  uint128_t_gpu *A, uint128_t_gpu *C, uint128_t_gpu *B, uint128_t_gpu *MATMUL_TABLE_REDUCTION, int M, int K, int N) {
    ...
    for (int i = 0; i < BLOCK_TILE_K; i++) {
      ...
      for (int jj = 0; jj < THREAD_BLOCK_H; jj++) {
        for (int kk = 0; kk < THREAD_BLOCK_W; kk++) {
          C_frag_local[jj][kk] = add_uint128(C_frag_local[jj][kk], mul_uint128(A_frag_local[jj], B_frag_local[kk]));
        }
      }
    }
  }
  ...
  for (int j = 0; j < THREAD_BLOCK_W; j++) {
    for (int i = 0; i < THREAD_BLOCK_H; i++) {
      MATMUL_TABLE_REDUCTION[block_indx_k * k_stride + block_C_indx_start + thread_indx_y * THREAD_BLOCK_H * N +
                             thread_indx_x * THREAD_BLOCK_W + i * N + j] = C_frag_local[i][j];
    }
  }
}

__global__ void GEMM128_reduction_kernel(
  uint128_t_gpu *MATMUL_TABLE_REDUCTION, uint128_t_gpu *out, int M, int K, int N) {
...
void GEMM128(uint128_t_gpu *A, uint128_t_gpu *C, uint128_t_gpu *B, int M, int K, int N, cudaStream_t s) {
