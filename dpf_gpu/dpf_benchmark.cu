...
int main() {
  ...
  if (PRF_METHOD == DUMMY) {
    ...
    check_correct(cw_cpu.data(), B_cpu.data(), NN, KK,
      DPF_STRATEGY == DPF_BREADTH_FIRST || DPF_STRATEGY == DPF_HYBRID || DPF_STRATEGY == DPF_COOP);
#endif
  }
  ...
  printf(
    "{'entries': %d, 'entry_size_ints': %d, 'batch_size': %d,"
    "'prf_method': '%s', 'dpf_strategy': '%s', "
    "'latency_ms' : %f, 'throughput_queries_per_ms' : %f,"
    "'fuses_matmul' : %d, 'performs_matmul' : %d}\n",
    KK, MM, NN, get_PRF_method().c_str(), get_DPF_strategy().c_str(), ms_latency, throughput_per_query, FUSES_MATMUL,
    PERFORM_MATMUL);
